...
<xsl:stylesheet version="1.1" xmlns="http://www.w3.org/1999/xhtml"
  ...
  <xsl:template match="presto:dir[@base]">
    ...
    <xsl:apply-templates select="*[1]"/>
    <div
      style="border: 1px solid #EEEEEE; font-family: -moz-fixed; color: grey;">
      [
      <xsl:for-each select="*[2]/presto:str">
        <xsl:sort order="ascending"/><a
          href="?PRESTo=async&amp;prompt={$base}.{.}">
        <xsl:value-of select="."/></a>, </xsl:for-each>]</div>
    <table class="presto-map">
      <xsl:for-each select="*[3]/presto:item">
        <xsl:sort select="*[1]" order="ascending"
          case-order="lower-first"/>
        <tr class="presto-map-item">
          <td class="presto-map-key"><a
              href="?PRESTo=async&amp;prompt=xdir({$base}.{*[1]})"
  ...
  <xsl:template match="presto:eval">
    <div class="presto-prompt-menu"><a
        href="?PRESTo=async&amp;prompt=xdir()">xdir</a> |<a>
      <xsl:attribute name="href">?</xsl:attribute>presto</a> </div>
    <form class="presto-prompt-form" method="GET"
      action="{/presto:PRESTo/@presto-path}">
      <input name="prompt" type="text" size="82"
        value="{/presto:PRESTo/@prompt}"/>
      <input name="PRESTo" type="SUBMIT"
        value="{/presto:PRESTo/@PRESTo}"/>
    </form>
    <div class="presto-line">
      <span
        style="padding-left: 4px; font-family: -moz-fixed; color: grey;">
  ...
  <xsl:template match="presto:exec">
    ...
    <div class="presto-line">
      <span
        style="padding-left: 4px; font-family: -moz-fixed; color: blue;">
        <xsl:value-of select="/presto:PRESTo/@prompt"/>
      </span>
    </div>
    <div class="presto-prompt-return">
  ...
  <xsl:template match="presto:excp">
    ...
    <div class="presto-line">
      <span
        style="padding-left: 4px; font-family: -moz-fixed; color: red;">
        <xsl:value-of select="/presto:PRESTo/@prompt"/>
      </span>
    </div>
    <div class="presto-prompt-return">
  ...
  <xsl:template match="presto:presto">
    <div class="presto-prompt-menu"> <a
        href="?PRESTo=async&amp;prompt=xdir()">async</a>
    </div>
  </xsl:template>
  
  <xsl:template match="presto:async">
    <div class="presto-top">
      <h1 style="font-weight: bold;">Async
        <span style="color: #BBBBBB;">
          <xsl:value-of select="/presto:PRESTo/@presto-path"/>
        </span></h1>
  ...
  <xsl:template match="presto:sync">
    <div class="presto-top">
      <h1 style="font-weight: bold;">Sync
        <span style="color: #BBBBBB;">
          <xsl:value-of select="/presto:PRESTo/@presto-path"/>
        </span></h1>
      <div style="font-size: small;">Copyright 2005 Laurent A.V.
...
It provides styles for the PRESTo prompt component, allowing 
