...
<xsl:template match="pns:index|pns:context">
  <a 
    href="?PRESTo=articulate&amp;context={.}"
    ><xsl:value-of select="."/></a>,
</xsl:template>    
    
<xsl:template match="pns:index[@names]|pns:context[@names]">
  [ <xsl:apply-templates/><a 
    href="?PRESTo=articulate&amp;context={@names}"
    >...</a> ]
</xsl:template>    
...
<xsl:template
  ...
  <div>
    <xsl:apply-templates/> | <a
      href="?PRESTo=articulate&amp;context={.}">index</a></div>
</xsl:template>
...
<xsl:template
  ...
  <div>
    <xsl:apply-templates/> | <a
      href="?PRESTo=articulate&amp;context={@names}">index</a>
    </div>
</xsl:template>

<xsl:template match="pns:articulate|presto:presto">
  ...
  <div class="presto-body">
    <form action="{/presto:PRESTo/@presto-path}" method="GET">
      ...
      <div class="presto-line">
        <input name="predicate" type="text" size="62" value="sat"/>
      </div>
    </form>
  </div>
